...
from database.db import async_session, POSHMARK_CA
...
async def add_data_to_db(session: AsyncSession, link, seller_name, price, items_count, items_sold):
    result = await session.execute(select(POSHMARK_CA).filter_by(seller_name=seller_name))
    existing_entry = result.scalars().first()

    if existing_entry is None:  # Если записи нет, добавляем новую
        poshmark_entry = POSHMARK_CA(
            price=price,
            link=link,
        ...
        print(f"Товар {link} добавлен в POSHMARK_CA.")
    else:
        print(f"Объявление с seller_name '{seller_name}' уже существует в базе данных.")

async def parse_poshmark():
    urls = [
        "https://poshmark.ca/category/Men?availability=available&sort_by=added_desc&all_size=true&my_size=false",
        "https://poshmark.ca/category/Women?availability=available&sort_by=added_desc&all_size=true&my_size=false",
        "https://poshmark.ca/category/Kids?availability=available&sort_by=added_desc&all_size=true&my_size=false",
        "https://poshmark.ca/category/Home?availability=available&sort_by=added_desc&all_size=true&my_size=false",
        "https://poshmark.ca/category/Electronics?availability=available&sort_by=added_desc&all_size=true&my_size=false",
        "https://poshmark.ca/category/Women-Makeup?availability=available&sort_by=added_desc&all_size=true&my_size=false",
        "https://poshmark.ca/category/Women-Jewelry?availability=available&sort_by=added_desc&all_size=true&my_size=false"
    ]
            ...
            for url in urls:
                try:
                    ...
                    for item in all_items:
                        ...
                        if price_text:
                            price = float(price_text[0].strip().replace('C$', '').replace(',', ''))
                        else:
                            price = 0.0
