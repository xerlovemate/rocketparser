...
from database.db import async_session, LALAFO_RS


async def save_to_db(seller_name, phone_number, price, views_count, reg_date, link):
    async with async_session() as session:
        result = await session.execute(select(LALAFO_RS).filter_by(seller_name=seller_name))
        existing_entry = result.scalars().first()
        if not seller_name:
            return
        if existing_entry is None:
            new_record = LALAFO_RS(
...
url_list = ['https://lalafo.rs/serbia',
            'https://lalafo.rs/serbia/personalni-proizvodi?sort_by=newest',
            'https://lalafo.rs/serbia/za-decu-2?sort_by=newest',
            'https://lalafo.rs/serbia/namestaj-2?sort_by=newest',
            'https://lalafo.rs/serbia/kuca-i-basta?sort_by=newest',
            'https://lalafo.rs/serbia/elektronika?sort_by=newest',
            'https://lalafo.rs/serbia/sport-i-hobi?sort_by=newest',
            'https://lalafo.rs/serbia/vozila?sort_by=newest',
            'https://lalafo.rs/serbia/kucni-ljubimci-2?sort_by=newest'
            ]


def clean_post_url(post_url):
    match = re.search(r"(https://lalafo.rs/[a-z-]+)/ads/.*?-(\d+)$", post_url)
...
def format_date(date_str):
    try:
        date_obj = datetime.strptime(date_str.split("od ")[1], '%d.%m.%Y')
