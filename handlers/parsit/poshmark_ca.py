...
import handlers
from database.db import async_session, User, Preset, POSHMARK_CA
...
@router.callback_query(F.data == 'parse_poshmark_ca')
async def parse_poshmark_ca(callback: CallbackQuery):
    ...
    async with async_session() as session:
        ...
        if user and user.preset_id:
            ...
            if preset:
                ...
                # Создаем инлайн клавиатуру для выбора даты
                date_kb = InlineKeyboardMarkup(inline_keyboard=[
                    [InlineKeyboardButton(text=today_str, callback_data=f"poshmark_ca_parse_date_{today_str}"),
                     InlineKeyboardButton(text=yesterday_str, callback_data=f"poshmark_ca_parse_date_{yesterday_str}")],
                    [InlineKeyboardButton(text=two_days_ago_str, callback_data=f"poshmark_ca_parse_date_{two_days_ago_str}"),
                     InlineKeyboardButton(text=three_days_ago_str, callback_data=f"poshmark_ca_parse_date_{three_days_ago_str}")],
...
async def calculate_available_items_poshmark_ca(preset, user, selected_date=None):
    filters = []

    if preset.min_reg_day:
        filters.append(POSHMARK_CA.reg_date >= preset.min_reg_day)
    if preset.max_reg_day:
        filters.append(POSHMARK_CA.reg_date <= preset.max_reg_day)

    if preset.price_diapazone:
        price_min, price_max = map(int, preset.price_diapazone.split('-'))
        filters.append(POSHMARK_CA.price >= price_min)
        filters.append(POSHMARK_CA.price <= price_max)

    if preset.max_views is not None:
        filters.append(POSHMARK_CA.views <= preset.max_views)
    if preset.max_posts is not None:
        filters.append(POSHMARK_CA.items_count <= preset.max_posts)
    if preset.max_sold is not None:
        filters.append(POSHMARK_CA.items_sold <= preset.max_sold)
    if preset.max_bought is not None:
        filters.append(POSHMARK_CA.items_bought <= preset.max_bought)

    if user.only_with_numbers:
        filters.append(POSHMARK_CA.is_phone_number == True)
    if user.only_with_delivery:
        filters.append(POSHMARK_CA.delivery == True)
    if user.find_without_rating:
        filters.append(POSHMARK_CA.has_rating == False)

    if selected_date:
        filters.append(POSHMARK_CA.parse_date == selected_date)

    print(f"Filters applied: {filters}")

    if filters:
        async with async_session() as session:
            query = select(POSHMARK_CA).filter(*filters)
        ...
        async with async_session() as session:
            result = await session.execute(select(POSHMARK_CA))
            ads = result.scalars().all()
...
class Form_Poshmark_ca(StatesGroup):
    waiting_for_quantity = State()


@router.callback_query(F.data.startswith("poshmark_ca_parse_date_"))
async def parse_date_poshmark_ca(callback: CallbackQuery, state: FSMContext):
    tg_id = callback.from_user.id
        ...
        if user and user.preset_id:
            ...
            if preset:
                ...
                markup = InlineKeyboardMarkup(inline_keyboard=[
                    [InlineKeyboardButton(text='◀️ Назад', callback_data='parse_poshmark_ca')]
                ])
                ...
                await state.update_data(selected_date=selected_date)
                await state.set_state(Form_Poshmark_ca.waiting_for_quantity)
...
@router.message(Form_Poshmark_ca.waiting_for_quantity)
async def handle_quantity_input_posh_ca(message: Message, state: FSMContext):
    try:
        ...
        async with async_session() as session:
            ...
            if user and user.preset_id:
                ...
                if preset:
                    available_items = await calculate_available_items_poshmark_ca(preset, user)
                    if quantity <= available_items:
                        ...
                        markup = InlineKeyboardMarkup(inline_keyboard=[
                            [InlineKeyboardButton(text=f'Оплатить {total_price} USDT', callback_data='poshmark_ca_pay')],
                            [InlineKeyboardButton(text='Отмена', callback_data='parse_poshmark_ca')]
                        ])
...
async def save_ads_to_txt_and_remove_poshmark_ca(preset, user, selected_date=None, num_to_write=10):
    filters = []
    ...
    async with async_session() as session:
        ...
        with open("объявления.txt", "a") as file:
            for ad in ads_to_write:
                ...
                await session.execute(delete(POSHMARK_CA).where(POSHMARK_CA.id == ad.id))
            await session.commit()

        print(f"Written {len(ads_to_write)} ads to file and removed from DB.")


async def save_2ads_to_txt_and_remove_poshmark_ca(preset, user, selected_date=None, num_to_write=10):
    filters = []
...
@router.callback_query(F.data == 'poshmark_ca_pay')
async def handle_payment_poshmark_ca(callback: CallbackQuery, state: FSMContext):
    tg_id = callback.from_user.id
    user_data = await state.get_data()
    ...
    async with async_session() as session:
        ...
        await save_2ads_to_txt_and_remove_poshmark_ca(preset, user, selected_date, num_to_write=quantity)
        await save_ads_to_txt_and_remove_poshmark_ca(preset, user, selected_date, num_to_write=quantity)
