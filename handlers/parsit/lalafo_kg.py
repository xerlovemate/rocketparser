...
import handlers
from database.db import async_session, User, Preset, POSHMARK_COM, LALAFO_KG
...
@router.callback_query(F.data == 'parse_lalafo_kg')
async def parse_lalafo_kg(callback: CallbackQuery):
        ...
        if user and user.preset_id:
            ...
            if preset:
                ...
                date_kb = InlineKeyboardMarkup(inline_keyboard=[
                    [InlineKeyboardButton(text=today_str, callback_data=f"lalafo_kg_parse_date_{today_str}"),
                     InlineKeyboardButton(text=yesterday_str, callback_data=f"lalafo_kg_parse_date_{yesterday_str}")],
                    [InlineKeyboardButton(text=two_days_ago_str, callback_data=f"lalafo_kg_parse_date_{two_days_ago_str}"),
                     InlineKeyboardButton(text=three_days_ago_str, callback_data=f"lalafo_kg_parse_date_{three_days_ago_str}")],
...
async def calculate_available_items_lalafo_kg(preset, user, selected_date=None):
    filters = []

    if preset.min_reg_day:
        filters.append(LALAFO_KG.reg_date >= preset.min_reg_day)
    if preset.max_reg_day:
        filters.append(LALAFO_KG.reg_date <= preset.max_reg_day)

    if preset.price_diapazone:
        price_min, price_max = map(int, preset.price_diapazone.split('-'))
        filters.append(LALAFO_KG.price >= price_min)
        filters.append(LALAFO_KG.price <= price_max)

    if preset.max_views is not None:
        filters.append(LALAFO_KG.views <= preset.max_views)
    if preset.max_posts is not None:
        filters.append(LALAFO_KG.items_count <= preset.max_posts)
    if preset.max_sold is not None:
        filters.append(LALAFO_KG.items_sold <= preset.max_sold)
    if preset.max_bought is not None:
        filters.append(LALAFO_KG.items_bought <= preset.max_bought)

    if user.only_with_numbers:
        filters.append(LALAFO_KG.is_phone_number == True)
    if user.only_with_delivery:
        filters.append(LALAFO_KG.delivery == True)
    if user.find_without_rating:
        filters.append(LALAFO_KG.has_rating == False)

    if selected_date:
        filters.append(LALAFO_KG.parse_date == selected_date)

    print(f"Filters applied: {filters}")

    if filters:
        async with async_session() as session:
            query = select(LALAFO_KG).filter(*filters)
        ...
        async with async_session() as session:
            result = await session.execute(select(LALAFO_KG))
            ads = result.scalars().all()
...
class Form_Lalafo_kg(StatesGroup):
    waiting_for_quantity = State()


@router.callback_query(F.data.startswith("lalafo_kg_parse_date_"))
async def parse_date_lalafo_kg(callback: CallbackQuery, state: FSMContext):
    tg_id = callback.from_user.id
    ...
    async with async_session() as session:
        ...
        if user and user.preset_id:
            ...
            if preset:
                ...
                available_items = await calculate_available_items_lalafo_kg(preset, user, selected_date)
                ...
                markup = InlineKeyboardMarkup(inline_keyboard=[
                    [InlineKeyboardButton(text='◀️ Назад', callback_data='parse_lalafo_kg')]
                ])
                ...
                await state.update_data(selected_date=selected_date)
                await state.set_state(Form_Lalafo_kg.waiting_for_quantity)
...
@router.message(Form_Lalafo_kg.waiting_for_quantity)
async def handle_quantity_input_lalafo_kg(message: Message, state: FSMContext):
    try:
        ...
        async with async_session() as session:
            ...
            if user and user.preset_id:
                ...
                if preset:
                    available_items = await calculate_available_items_lalafo_kg(preset, user)
                    if quantity <= available_items:
                        ...
                        markup = InlineKeyboardMarkup(inline_keyboard=[
                            [InlineKeyboardButton(text=f'Оплатить {total_price} USDT', callback_data='lalafo_kg_pay')],
                            [InlineKeyboardButton(text='Отмена', callback_data='parse_lalafo_kg')]
                        ])
...
async def save_ads_to_txt_and_remove_lalafo_kg(preset, user, selected_date=None, num_to_write=10):
    filters = []
    ...
    async with async_session() as session:
        ...
        with open("объявления.txt", "a") as file:
            for ad in ads_to_write:
                ...
                await session.execute(delete(LALAFO_KG).where(LALAFO_KG.id == ad.id))
            await session.commit()

        print(f"Written {len(ads_to_write)} ads to file and removed from DB.")


@router.callback_query(F.data == 'lalafo_kg_pay')
async def handle_payment_lalafo_kg(callback: CallbackQuery, state: FSMContext):
    tg_id = callback.from_user.id
    user_data = await state.get_data()
    ...
    async with async_session() as session:
        ...
        await save_ads_to_txt_and_remove_lalafo_kg(preset, user, selected_date, num_to_write=quantity)
