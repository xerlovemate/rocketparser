...
import handlers
from database.db import async_session, User, Preset, POSHMARK_COM, LALAFO_RS
...
@router.callback_query(F.data == 'choose_lalafo')
async def choose_poshmark(callback: CallbackQuery):
    text = '<b>Выберите сервис:</b>'
    markup = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text='🇷🇸 LALAFO.RS', callback_data='parse_lalafo_rs'),
         InlineKeyboardButton(text='🇰🇬 LALAFO.KG', callback_data='parse_lalafo_kg')],
...
@router.callback_query(F.data == 'parse_lalafo_rs')
async def parse_lalafo_rs(callback: CallbackQuery):
        ...
        if user and user.preset_id:
            ...
            if preset:
                ...
                date_kb = InlineKeyboardMarkup(inline_keyboard=[
                    [InlineKeyboardButton(text=today_str, callback_data=f"lalafo_rs_parse_date_{today_str}"),
                     InlineKeyboardButton(text=yesterday_str, callback_data=f"lalafo_rs_parse_date_{yesterday_str}")],
                    [InlineKeyboardButton(text=two_days_ago_str, callback_data=f"lalafo_rs_parse_date_{two_days_ago_str}"),
                     InlineKeyboardButton(text=three_days_ago_str, callback_data=f"lalafo_rs_parse_date_{three_days_ago_str}")],
...
async def calculate_available_items_lalafo_rs(preset, user, selected_date=None):
    filters = []

    if preset.min_reg_day:
        filters.append(LALAFO_RS.reg_date >= preset.min_reg_day)
    if preset.max_reg_day:
        filters.append(LALAFO_RS.reg_date <= preset.max_reg_day)

    if preset.price_diapazone:
        price_min, price_max = map(int, preset.price_diapazone.split('-'))
        filters.append(LALAFO_RS.price >= price_min)
        filters.append(LALAFO_RS.price <= price_max)

    if preset.max_views is not None:
        filters.append(LALAFO_RS.views <= preset.max_views)
    if preset.max_posts is not None:
        filters.append(LALAFO_RS.items_count <= preset.max_posts)
    if preset.max_sold is not None:
        filters.append(LALAFO_RS.items_sold <= preset.max_sold)
    if preset.max_bought is not None:
        filters.append(LALAFO_RS.items_bought <= preset.max_bought)

    if user.only_with_numbers:
        filters.append(LALAFO_RS.is_phone_number == True)
    if user.only_with_delivery:
        filters.append(LALAFO_RS.delivery == True)
    if user.find_without_rating:
        filters.append(LALAFO_RS.has_rating == False)

    if selected_date:
        filters.append(LALAFO_RS.parse_date == selected_date)

    print(f"Filters applied: {filters}")

    if filters:
        async with async_session() as session:
            query = select(LALAFO_RS).filter(*filters)
        ...
        async with async_session() as session:
            result = await session.execute(select(LALAFO_RS))
            ads = result.scalars().all()
...
class Form_Lalafo_rs(StatesGroup):
    waiting_for_quantity = State()


@router.callback_query(F.data.startswith("lalafo_rs_parse_date_"))
async def parse_date_lalafo_rs(callback: CallbackQuery, state: FSMContext):
    tg_id = callback.from_user.id
    ...
    async with async_session() as session:
        ...
        if user and user.preset_id:
            ...
            if preset:
                ...
                available_items = await calculate_available_items_lalafo_rs(preset, user, selected_date)
                ...
                markup = InlineKeyboardMarkup(inline_keyboard=[
                    [InlineKeyboardButton(text='◀️ Назад', callback_data='parse_lalafo_rs')]
                ])
                ...
                await state.update_data(selected_date=selected_date)
                await state.set_state(Form_Lalafo_rs.waiting_for_quantity)
...
@router.message(Form_Lalafo_rs.waiting_for_quantity)
async def handle_quantity_input_lalafo_rs(message: Message, state: FSMContext):
    try:
        ...
        async with async_session() as session:
            ...
            if user and user.preset_id:
                ...
                if preset:
                    available_items = await calculate_available_items_lalafo_rs(preset, user)
                    if quantity <= available_items:
                        ...
                        markup = InlineKeyboardMarkup(inline_keyboard=[
                            [InlineKeyboardButton(text=f'Оплатить {total_price} USDT', callback_data='lalafo_rs_pay')],
                            [InlineKeyboardButton(text='Отмена', callback_data='parse_lalafo_rs')]
                        ])
...
async def save_ads_to_txt_and_remove_lalafo_rs(preset, user, selected_date=None, num_to_write=10):
    filters = []
        ...
        with open("объявления.txt", "a") as file:
            for ad in ads_to_write:
                if ad.phone_number:
                    ...
                    line = f"{ad.link}, {phone_number}\n"
                else:
                    line = f"{ad.link}\n"

                file.write(line)

                await session.execute(delete(LALAFO_RS).where(LALAFO_RS.id == ad.id))
            await session.commit()

        print(f"Written {len(ads_to_write)} ads to file and removed from DB.")


@router.callback_query(F.data == 'lalafo_rs_pay')
async def handle_payment_lalafo_rs(callback: CallbackQuery, state: FSMContext):
    tg_id = callback.from_user.id
    user_data = await state.get_data()
    ...
    async with async_session() as session:
        ...
        await save_ads_to_txt_and_remove_lalafo_rs(preset, user, selected_date, num_to_write=quantity)
