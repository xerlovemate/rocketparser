...
import handlers
from database.db import async_session, User, Preset, POSHMARK_COM
...
@router.callback_query(F.data == 'choose_poshmark')
async def choose_poshmark(callback: CallbackQuery):
    text = '<b>Выберите сервис:</b>'
    markup = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text='🇺🇸 POSHMARK.COM', callback_data='parse_poshmark_com'),
         InlineKeyboardButton(text='🇨🇦 POSHMARK.CA', callback_data='parse_poshmark_ca')],
...
@router.callback_query(F.data == 'parse_poshmark_com')
async def parse_poshmark_com(callback: CallbackQuery):
        ...
        if user and user.preset_id:
            ...
            if preset:
                ...
                date_kb = InlineKeyboardMarkup(inline_keyboard=[
                    [InlineKeyboardButton(text=today_str, callback_data=f"poshmark_com_parse_date_{today_str}"),
                     InlineKeyboardButton(text=yesterday_str, callback_data=f"poshmark_com_parse_date_{yesterday_str}")],
                    [InlineKeyboardButton(text=two_days_ago_str, callback_data=f"poshmark_com_parse_date_{two_days_ago_str}"),
                     InlineKeyboardButton(text=three_days_ago_str, callback_data=f"poshmark_com_parse_date_{three_days_ago_str}")],
...
async def calculate_available_items_poshmark_com(preset, user, selected_date=None):
    filters = []

    if preset.min_reg_day:
        filters.append(POSHMARK_COM.reg_date >= preset.min_reg_day)
    if preset.max_reg_day:
        filters.append(POSHMARK_COM.reg_date <= preset.max_reg_day)

    if preset.price_diapazone:
        price_min, price_max = map(int, preset.price_diapazone.split('-'))
        filters.append(POSHMARK_COM.price >= price_min)
        filters.append(POSHMARK_COM.price <= price_max)

    if preset.max_views is not None:
        filters.append(POSHMARK_COM.views <= preset.max_views)
    if preset.max_posts is not None:
        filters.append(POSHMARK_COM.items_count <= preset.max_posts)
    if preset.max_sold is not None:
        filters.append(POSHMARK_COM.items_sold <= preset.max_sold)
    if preset.max_bought is not None:
        filters.append(POSHMARK_COM.items_bought <= preset.max_bought)

    if user.only_with_numbers:
        filters.append(POSHMARK_COM.is_phone_number == True)
    if user.only_with_delivery:
        filters.append(POSHMARK_COM.delivery == True)
    if user.find_without_rating:
        filters.append(POSHMARK_COM.has_rating == False)

    if selected_date:
        filters.append(POSHMARK_COM.parse_date == selected_date)

    print(f"Filters applied: {filters}")

    if filters:
        async with async_session() as session:
            query = select(POSHMARK_COM).filter(*filters)
        ...
        async with async_session() as session:
            result = await session.execute(select(POSHMARK_COM))
            ads = result.scalars().all()
...
class Form_Poshmark_com(StatesGroup):
    waiting_for_quantity = State()


@router.callback_query(F.data.startswith("poshmark_com_parse_date_"))
async def parse_date_poshmark_com(callback: CallbackQuery, state: FSMContext):
    tg_id = callback.from_user.id

    selected_date = callback.data.split("_")[4]
    ...
    async with async_session() as session:
        ...
        if user and user.preset_id:
            ...
            if preset:
                ...
                available_items = await calculate_available_items_poshmark_com(preset, user, selected_date)
                ...
                markup = InlineKeyboardMarkup(inline_keyboard=[
                    [InlineKeyboardButton(text='◀️ Назад', callback_data='parse_poshmark_com')]
                ])
                ...
                await state.update_data(selected_date=selected_date)
                await state.set_state(Form_Poshmark_com.waiting_for_quantity)
...
@router.message(Form_Poshmark_com.waiting_for_quantity)
async def handle_quantity_input_posh_com(message: Message, state: FSMContext):
    try:
        ...
        async with async_session() as session:
            ...
            if user and user.preset_id:
                ...
                if preset:
                    available_items = await calculate_available_items_poshmark_com(preset, user)
                    if quantity <= available_items:
                        ...
                        markup = InlineKeyboardMarkup(inline_keyboard=[
                            [InlineKeyboardButton(text=f'Оплатить {total_price} USDT', callback_data='poshmark_com_pay')],
                            [InlineKeyboardButton(text='Отмена', callback_data='parse_poshmark_com')]
                        ])
...
async def save_ads_to_txt_and_remove_poshmark_com(preset, user, selected_date=None, num_to_write=10):
    filters = []
    ...
    async with async_session() as session:
        ...
        with open("объявления.txt", "a") as file:
            for ad in ads_to_write:
                ...
                await session.execute(delete(POSHMARK_COM).where(POSHMARK_COM.id == ad.id))
            await session.commit()

        print(f"Written {len(ads_to_write)} ads to file and removed from DB.")


async def save_2ads_to_txt_and_remove_poshmark_com(preset, user, selected_date=None, num_to_write=10):
    filters = []
    ...
    async with async_session() as session:
        ...
        with open("для_рассылок.txt", "a") as file:
            for ad in ads_to_write:

                line = f"{ad.link}|{ad.seller_name}\n"

                file.write(line)


@router.callback_query(F.data == 'poshmark_com_pay')
async def handle_payment_poshmark_com(callback: CallbackQuery, state: FSMContext):
    tg_id = callback.from_user.id
    user_data = await state.get_data()
    ...
    async with async_session() as session:
        ...
        await save_2ads_to_txt_and_remove_poshmark_com(preset, user, selected_date, num_to_write=quantity)
        await save_ads_to_txt_and_remove_poshmark_com(preset, user, selected_date, num_to_write=quantity)
        ...
        file_path2 = "для_рассылок.txt"

        input_file = FSInputFile(file_path2)
